...
class RetestEngine:
    ...
    async def create_and_execute_retest(
        ...
        if still_vulnerable:
            ...
            verdict = "Vulnerability is no longer reproducible. Specialized validation confirmed security remediation."
            logger.info("Retest %s PASSED — finding %s marked FIXED", retest_id, finding.id)
        ...
        comparison_data = {
            ...
            "tested_at": now_iso,
            "target_url": target_url,
            "is_still_vulnerable": still_vulnerable,
            "verdict": verdict,
    ...
    async def check_clean_state(self, db: AsyncSession, scan_id: str) -> dict:
        ...
        return {
            ...
            "clean": len(open_findings) == 0,
            "status": "CLEAN" if len(open_findings) == 0 else "OPEN_FINDINGS_REMAIN",
        }
