...
class Settings(BaseSettings):
    ...
    max_concurrent_hosts: int = 16
    max_concurrent_scans: int = int(os.getenv("MAX_CONCURRENT_SCANS", "8"))
    max_pending_scans: int = int(os.getenv("MAX_PENDING_SCANS", "100"))
    max_browser_captures: int = 2
    browser_capture_enabled: bool = False
    nuclei_enabled: bool = True
    nuclei_concurrency: int = int(os.getenv("NUCLEI_CONCURRENCY", "25"))
    nuclei_rate_limit: int = int(os.getenv("NUCLEI_RATE_LIMIT", "50"))
    nuclei_timeout_seconds: float = float(os.getenv("NUCLEI_TIMEOUT_SECONDS", "30.0"))
    nuclei_tags: str = os.getenv("NUCLEI_TAGS", "cve,misconfig,exposure,tech,sqli,xss,auth,lfi")
    ...
    # auto preserves legacy LLM_MODEL=combo; explicit modes remove ambiguity.
    llm_routing_mode: str = os.getenv("LLM_ROUTING_MODE", "auto")
